...
class Validator:
    ...
    def validate_instruction(self, instruction: Union[Dict, List]) -> bool:
        ...
        if isinstance(instruction, list):
            ...
            return all(Path(path).exists() for path in instruction if isinstance(path, str))
        ...
        for field in time_fields:
            if field in action_data:
                if not self._validate_time_format(action_data[field]):
                    logger.error(f"Invalid time format in {field}: {action_data[field]}")
                    return False
...
#   # from .state_manager import StateManager  # This doesn't exist in core
